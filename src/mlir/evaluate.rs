...
impl MlIrModule {
  ...
  pub fn evaluate(&mut self, mangle: &Mangle, memory: &mut Memory) -> Result<()> {
  ...
  pub fn evaluate_function(&self, mangle: &Mangle, memory: &mut Memory) -> Result<ConstValue> {
    let Some(Block {
    ...
    Ok(ConstValue::Function {
      name: mangle.clone(),
      parameters: vec![Type::Ambiguous; parameters.len()],
      returns: Type::Ambiguous,
    })
  }

  pub fn evaluate_constant(&self, mangle: &Mangle, memory: &mut Memory) -> Result<ConstValue> {
    ...
    loop {
      ...
      match instr.kind.clone() {
        ...
        Field(field) => {
          ...
          let position = member_names.iter().position(|s| s == &field).ok_or(lint(
        ...
        Call { arity, spans } => {
          ...
          let ConstValue::Function {
            ...
            return Err(lint(TypeLint::NonFunctionCall, span, &[format!(
              "{}",
              top.type_of()
            )]));
          };
          arguments
            ...
            .map(|((argument, expect), span)| {
              if &argument.type_of() != expect {
                ...
                Err(lint(TypeLint::TypeMismatch, *span, &[
                  format!("{expect}"),
                  format!("{}", argument.type_of()),
                ]))
              }
            })
            .try_collect::<Vec<_>>()?;
  ...
  fn execute_builtin(
    ...
    match builtin {
      Builtin::PrintString => {
        let ConstValue::String { address, length } = stack.pop().unwrap() else {
          panic!();
        };
