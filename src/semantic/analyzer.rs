...
use crate::{
  ...
  error,
  semantic::{
    consteval::{ConstValue, parse_int_literal, parse_real_literal},
    primitives::Primitive,
  },
};

use super::{
  Analyzer, MAIN_MANGLE, Mangle, Type, ir::*, mangle_builtin,
  operators::OpTable,
};
use NodeKind as n;
...
impl Analyzer {
  ...
  pub fn prelude(&mut self) {
    ...
    // Primitive STD library
    const PRINT_STRING: &str = "print_string";
    self._name_to_symbol.insert(PRINT_STRING.into(), Symbol {
    ...
    self
      ...
      .insert(mangle_builtin(PRINT_STRING), Type::Function {
        mangle: mangle_builtin(PRINT_STRING),
        param_names: vec!["str".into()],
        param_types: vec![Type::Type(Primitive::string.promote().into())],
        return_type: Type::Type(Primitive::nothing.promote().into()).into(),
      });
  }

  pub fn print_table(&self) {
    let new_table = self._mangle_to_type.clone();
    println!("{new_table:#?}");
  }
  ...
  fn hook_main_fn(&mut self, node: Node) -> Result<Node> {
    let err = error!(
      "The name 'main' is reserved in the global scope. It must refer to a \
       function with no inputs which returns nothing."
    )
    .span(&node.span);
    let Type::Function {
      param_names,
    ...
    if let Ok(Type::Prim(Primitive::nothing)) = return_type.unwrap_type_name() {
    } else {
      return err;
    }
    let type_ = Type::Function {
      mangle: MAIN_MANGLE.into(),
      param_names,
      param_types: vec![],
      return_type: Type::Type(Primitive::nothing.promote().into()).into(),
    };
    self
      ._mangle_to_type
      .insert(MAIN_MANGLE.into(), type_.clone());
    self.has_main = true;
    Ok(Node {
      span: node.span,
      type_,
  ...
  fn analyze_statement(&mut self, stmt: Statement) -> Result<Node> {
    ...
    Ok(match stmt.kind {
      s::Declaration {
        ...
        let is_main_fn = name == "main" && self.scope_depth == 0;
        let value = self.analyze_expression(value)?;
        let value = if is_main_fn {
          self.hook_main_fn(value)?
        } else {
          value
        };
        let mangle = if is_main_fn {
          self._name_to_symbol.insert("main".into(), Symbol {
            mangle: MAIN_MANGLE.into(),
            scope_depth: 0,
            is_constant: true,
          });
          self
            ._mangle_to_type
            .insert(MAIN_MANGLE.into(), Type::Ambiguous);
          Ok(MAIN_MANGLE.to_string())
        } else if !is_constant {
  ...
  fn analyze_expression(&mut self, expr: Expression) -> Result<Node> {
    ...
    let (type_, kind) = match expr.kind {
      e::Immediate(immediate) => {
        let type_ = immediate.type_of().promote();
        ...
        (type_, n::Immediate(const_val))
      },
      e::Identifier { name } => {
        let symbol = self.name_to_symbol(&name)?;
