...
pub fn check_structs(env: &Environment, ir: &IrModule, ptr: IrPtr) -> Result<Vec<TypeConstraint>> {
        ...
        if let IrKind::Field { of, index } = &node.kind {
            ...
            // LHS failed to infer, try to use context clues
                ...
                match &structs
                    ...
                    .filter(|(_, names, _)| names.contains(index))
                    .collect::<Vec<_>>()[..]
                {
                    // No candidates found
                    [] => {
                    ...
                    [(mangle, names, types)] => {
                        ...
                        constraints.extend_from_slice(&[
                            ...
                            TypeConstraint(new_of_t, Type::TypeVariable(tv).to_ref(), node.span),
                        ]);
                    }
