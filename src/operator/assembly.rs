...
impl OpTable {
  ...
  pub fn get_binary_defs(&self) -> Vec<(BinaryOp, Type, Type, Type)> {
    self
      .binary_map
      .iter()
      .map(|(key, def)| {
  ...
  pub fn get_unary_defs(&self) -> Vec<(UnaryOp, Type, Type)> {
    self
      .unary_map
      .iter()
      .map(|(key, def)| (key.op, key.on.clone(), def.produces.clone()))
      .collect::<Vec<_>>()
  }
