...
use crate::semantic::{ConstValue, Module, Node};
use crate::{
  err::*,
  error,
    ...
    .map(&mut |n| {
      ...
      } = &n.kind
      {
        if !set.contains(mangle) {
          ...
          find_dependencies(map, map.get(mangle).unwrap(), set);
        }
      } else if let NodeKind::ConstValue(ConstValue::Function(mangle)) = &n.kind
      {
        let func = map.get(mangle).unwrap();
        ...
        param_types
          .into_iter()
          .for_each(|t| find_dependencies(map, t, set));
        find_dependencies(map, returns, set);
      }
      Ok(())
    })
    .unwrap();
